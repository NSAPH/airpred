...
#' @import RANN
#' @export
gen_weights <- function(orig, query, k=4, threshold) {
    neighbors <- nn2(data = orig, query = query, k = k)

    weight_matrix <- process_weights(neighbors, nrow(orig), nrow(query), threshold)

    return(weight_matrix)
}


process_weights <- function(neighbors, orig_len, query_len, threshold) {
  ...
  for (i in 1:query_len) {
    for (j in 1:nrow(neighbors$nn.idx)) {
      if (neighbors$nn.dists[i,j] <= threshold && neighbors$nn.dists[i,j] != 0) {
        out[i,neighbors$nn.idx[i,j]] <- 1/neighbors$nn.dists[i,j]
      }
    }
  }
